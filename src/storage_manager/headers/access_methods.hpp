...
class Access_methods {
  ...
  public:
    ...
    class heap_scan {
      ...
      public:
        heap_scan(buffer_manager::buffer_pool &buff_pool)
            : HeapTable(), buff_pool(buff_pool) {
            HeapTable.push_back(curr_pid);
        };
        /* Does not return deleted rows  */
        access_methods_types::ScanResult
        scan(std::vector<size_t> &data_size_arr,
    ...
    bptree_leaf_insert(buffer_manager_types::Page *left_raw_page,
                       buffer_manager_types::Page *right_raw_page, int key,
                       heap_page_types::RID rid);

    std::optional<access_methods::split_res>
    bptree_internal_insert(buffer_manager_types::Page *left_raw_page,
                           buffer_manager_types::Page *right_raw_page,
                           btree_page_types::node_id child_pid, int key);

    access_methods::split_res
    bptree_internal_split(buffer_manager_types::Page *left_raw_page,
                          buffer_manager_types::Page *right_raw_page,
                          int                        *temp_keys,
                          heap_page_types::page_id   *temp_child_id);

    access_methods::split_res
    bptree_leaf_split(buffer_manager_types::Page *left_raw_page,
                      buffer_manager_types::Page *right_raw_page,
                      int *temp_keys, heap_page_types::RID *temp_rids);

    heap_page_types::RID
    bptree_scan(buffer_manager::buffer_pool   &buff_pool,
