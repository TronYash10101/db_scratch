...
class Seq_scan : public Operator {
  ...
  public:
    Seq_scan(const schema::tables_attrs      tn,
             access_methods::Access_methods &access_methods,
             buffer_manager::buffer_pool    &buff_pool,
             std::vector<size_t>            &data_size_arr,
             std::vector<access_methods_types::SUPORTED_COLUMN_TYPE> &col_types)
        : table_attr(tn), am(access_methods), buff_pool(buff_pool),
          heap_scan(buff_pool), data_size_arr(data_size_arr),
          col_types(col_types) {
    }
    ...
    access_methods_types::ScanResult next() override {
        ...
        access_methods_types::ScanResult res =
            heap_scan.scan(data_size_arr, col_types);
...
class Filter : public Operator {
  protected:
    ...
    access_methods_types::SARG parse_predicate(
        const access_methods_types::SUPORTED_COLUMN_TYPE &col_type) {
    ...
    std::optional<bool>
    match_sarg(std::optional<access_methods_types::row_t> res_row,
               size_t                                     row_no) {
  ...
  public:
    Filter(const schema::tables_attrs tn, Operator &op, bool have_predicate,
           parser_types::Predicate &predicate)
        : next_op(op), predicate(predicate), table_attr(tn),
          have_predicate(have_predicate) {};
    void init() override {
        if (have_predicate) {
    ...
    access_methods_types::ScanResult next() override {
        ...
        while (true) {
            ...
            if (res_row.scan_result.has_value()) {
                if (have_predicate &&
                    match_sarg(res_row.scan_result.value(), search_column)
                        .value()) {
                    return res_row;
                }
                if (!have_predicate) {
...
class Projection : public Operator {
  ...
  public:
    Projection(schema::tables_attrs tn, Operator &op,
               parser_types::SELECT_AST &ast)
        : next_op(op), ast(ast), table_attr(tn) {
    }
...
class Insert : public Operator {
  ...
  public:
    Insert(schema::tables_attrs &tn, Operator *next_op,
           parser_types::INSERT_AST &ast, access_methods::Access_methods &am,
           buffer_manager::buffer_pool &buff_pool,
           index_write::root_struct    &curr_root,
           std::vector<size_t>          data_size_arr)
        : next_op(next_op), ast(ast), am(am), buff_pool(buff_pool),
          curr_root(curr_root), data_size_arr(data_size_arr) {};
    ...
    access_methods_types::ScanResult next() override {
        ...
        if (curr_row < ast.values.size()) {
            if (auto v =
                    insert::create_entry(buff_pool, am, ast.values[curr_row],
                                         data_size_arr, &curr_root, false)) {
...
std::vector<access_methods_types::row_t>
select_plan(buffer_manager::buffer_pool    &buff_pool,
            access_methods::Access_methods &access_methods,
            schema::schema_manager &sch_man, parser_types::SELECT_AST &ast,
            std::string schema_name);

std::vector<access_methods_types::row_t>
insert_plan(buffer_manager::buffer_pool    &buff_pool,
            access_methods::Access_methods &access_methods,
