...
buffer_manager::buffer_pool::buffer_pool(const std::string &db_filename, const std::string &index_filename)
    : disk_operator(db_filename, index_filename, page_size), frames(buffer_size) {
    std::cout << "\nBUFFER CREATED\n";
}
    ...
    if (page != page_table.end()) {
        ...
        for (auto i = 0; i != frames.size(); ++i) {
            if (frames[i].page_id == INVALID_PAGE_ID) {
                ...
                disk_operator.read_page(pid, frames[i].page_data, HEAP_PAGE);
                std::cout << "\nNew Page Created\n";
                return &frames[i];
            }
        }
...
buffer_manager::frame_id buffer_manager::buffer_pool::page_replacement_policy() {
    int idx;
