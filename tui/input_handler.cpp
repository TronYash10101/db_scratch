...
void InputHandlers::Stdin_Handler::navigation_handle(Events &events) {
    ...
    if (Accordion *ptr = dynamic_cast<Accordion *>(active_component)) {
        if (dir_key == 'A') {
            ...
            } else if (!ptr->entry[ptr->which_selected].is_expanded && ptr->which_selected > 0) {
            ...
            if (ptr->entry[ptr->which_selected].is_expanded &&
                ...
                ptr->entry[ptr->which_selected].sub_child_selected += 1;
            } else if (!ptr->entry[ptr->which_selected].is_expanded && ptr->which_selected < ptr->entry.size() - 1) {
                ptr->which_selected += 1;
            }
        } else if (dir_key == '\n' || dir_key == '\r') {
